...
std::vector<std::string> Graph::bfs(const std::string& start) const {
	...
	if (!has_vertex(start)) return result; // start vertex must exist
...
std::vector<std::string> Graph::dfs(const std::string& start) const {
	...
	while (!s.empty()) {
        ...
        result.push_back(current);
		const auto& neighbors_list = neighbors(current);
		for (auto it = neighbors_list.rbegin(); it != neighbors_list.rend(); ++it) {
